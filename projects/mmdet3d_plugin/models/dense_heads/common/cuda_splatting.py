...
def render_depth_cuda2(
    ...
    b, num_pts = gaussian_means.shape[:2]
    fake_color = torch.ones((b, num_pts), 
                            dtype=gaussian_means.dtype, 
                            device=gaussian_means.device) * 255
    result = render_cuda(
        extrinsics,
        intrinsics,
        near,
        far,
        image_shape,
        torch.zeros((b, 3), dtype=gaussian_means.dtype, device=gaussian_means.device),
        gaussian_means,
        gaussian_covariances,
        repeat(fake_color, "b g -> b g c ()", c=3),
        gaussian_opacities,
